		...
		if (TSharedPtr<SGraphPanel> GraphPanel = GraphHandler->GetGraphPanel())
		{
			...
			if (IsAnyInputChordDown(BASettings->GroupMovementChords))
			{
				...
				if (!HoveredPin && HoveredNode)
				{
					...
					const TSet<UEdGraphNode*> NodesToMove = FBAUtils::GetNodeTree(AnchorNode.Get());
					DragNodeTransaction.Begin(NodesToMove);
				}
			}
			...
			if (IsAnyInputChordDown(BASettings->AdditionalDragNodesChords))
			{
				if (HoveredNode)
				{
					...
					// begin transaction
					DragNodeTransaction.Begin(GraphHandler->GetSelectedNodes(true));
				}
			}
		}
	}
...
bool FBAInputProcessor::OnKeyOrMouseUp(FSlateApplication& SlateApp, const FKey& Key)
{
	...
	{
		bBlocking = true;
		DragNodeTransaction.End(NSLOCTEXT("UnrealEd", "BADragNode", "BA Drag Node"));
	}

	if (IsAnyInputChordDown(BASettings->GroupMovementChords, Key))
	{
		AnchorNode = nullptr;
		DragNodeTransaction.End(NSLOCTEXT("UnrealEd", "BAGroupMoveNodes", "Group Move Nodes"));
	}
